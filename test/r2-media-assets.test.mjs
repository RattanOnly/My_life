...
test('post images are served from the R2 blog image path', async () => {
  ...
  assert.equal(r2ImageReferences, 44);
});
...
test('article and site music are served from the R2 blog music path', async () => {
  ...
  assert.match(combined, new RegExp(`${R2_ASSET_ORIGIN}/blog/music/blog-wind-soft-light\\.mp3`));
  assert.match(combined, new RegExp(`${R2_ASSET_ORIGIN}/blog/music/blog-it-is-okay-itsok\\.mp3`));
